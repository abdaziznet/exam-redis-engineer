...
This section is specific to `task-3` in this repository.

```bash
cd task-3
pip install -r requirements.txt
python3 create_task3_db.py
python3 semantic_router.py
```

Notes:
- `create_task3_db.py` creates the Redis database and updates connection details used by `semantic_router.py`.
- `semantic_router.py` builds the semantic router index and prints only the route name for each test query.
